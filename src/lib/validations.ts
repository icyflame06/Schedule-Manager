...
// Helper for sanitizing strings to prevent XSS
...
export const MeetingTypeSchema = z.object({
  title: z.string().min(1).max(100).transform(sanitizeString),
...
export const BookingSchema = z.object({
  ...
  guest_name: z.string().min(1).max(100).transform(sanitizeString),
  guest_email: z.string().email(),
  guest_notes: z.string().max(1000).transform(sanitizeString).optional(),
